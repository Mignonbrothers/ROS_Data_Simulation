...
ros2 pkg create --build-type ament_python lidar_sim --dependencies rclpy sensor_msgs
...
lidar_sim 폴더에 이동해서 그 폴더 안에서 lidar_publisher.py라는 파일을 만듭니다.

이제 Github에 있는 코드를 가져와서 적용합니다.
...
remain_main.py를 실행하기 전 UBUNTU_IP를 9번에서 입력하고 나온 결과를 써주세요.
...
remain_main.py와 remain_out_csv.py에 있는 mysql.connector.connect 함수 안에 있는 코드를 Mysql에서 똑같이 맞춰줘야 합니다.

mysql에는 깃헙 ROS_Python 폴더 안 sql_record_ros.sql에서 참조합니다.

13. 따라오느라 정말 고생하셨습니다. 이제 마지막 실행 과정입니다. 우분투 터미널 3개를 열어서 다음과 같은 명령어를 실행하세요
EX)

(우분투 터미널 1) ros2 run turtlesim turtlesim_node
